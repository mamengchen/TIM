...
class SocketApi {
    public:
        ...
        static void RecvMassage(int sock, std::string &message, struct sockaddr_in &peer)
        {
            char msg[1024];
            socklen_t len = sizeof(peer);
...
class Util {
public:
    static bool ReigsterEnter(std::string &n_, std::string &s_,
                              std::string &passwd)
    {
            std::string again;
    ...
    static bool LogoutDestroy(unsigned int &id, std::string &passwd)
    {
    ...
    static void SendMessage(int sock, const std::string &message, 
                            struct sockaddr_in &peer)
    {
        sendto(sock, message.c_str(), message.size(), 0, (struct sockaddr*)&peer, 
               sizeof(peer));
    }
    ...
    static void deleteUser(std::vector<std::string> &online, std::string &f)
    {
        for (auto it = online.begin(); it != online.end(); it++)
        {
            if (*it == f)
